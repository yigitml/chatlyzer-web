...
export default function Home() {
  ...
  return (
    <div className="min-h-screen text-foreground bg-background relative overflow-hidden flex flex-col selection:bg-primary selection:text-primary-foreground">
      ...
      {/* Navigation */}
      <header className="border-b-4 border-primary bg-background py-3 sm:py-4 px-4 md:px-8 flex flex-wrap gap-3 items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <Image src="/iconsvg.svg" alt="Chatlyzer" width={32} height={32} className="w-8 h-8 rounded-none border-2 border-primary shadow-[4px_4px_0px_0px_hsl(var(--primary))] bg-card p-0.5 shrink-0" />
          <span className="font-display font-black text-xl sm:text-2xl tracking-widest uppercase leading-none">Chatlyzer</span>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4 font-mono uppercase font-bold text-xs sm:text-sm ml-auto">
          <Link
            href="https://github.com/yigitml/chatlyzer-web"
            target="_blank"
            rel="noopener noreferrer"
      ...
      {/* Hero Section */}
      <main className="flex-1 container mx-auto px-4 mt-10 sm:mt-16 mb-16 sm:mb-20 text-center relative z-10 flex flex-col items-center">
        <div className="inline-flex flex-wrap items-center justify-center gap-2 border-2 border-primary bg-card text-card-foreground px-3 sm:px-4 py-3 mb-8 sm:mb-12 shadow-[4px_4px_0px_0px_hsl(var(--primary))] font-mono uppercase text-xs sm:text-sm font-bold rotate-[1deg] max-w-full">
        ...
        <h1 className="text-4xl sm:text-5xl md:text-[7rem] font-display font-black uppercase tracking-tighter mb-6 sm:mb-8 leading-[0.92] sm:leading-[0.9] text-balance">
          YOUR CHATS ARE<br />
          <span className="relative inline-block mt-2 bg-primary text-primary-foreground px-3 sm:px-6 py-2 border-4 border-primary shadow-[8px_8px_0px_0px_hsl(var(--foreground))] sm:shadow-[12px_12px_0px_0px_hsl(var(--foreground))] rotate-[-2deg] hover:rotate-[0deg] transition-transform duration-300">
            LOWKEY SUS
          </span>
        </h1>

        <p className="text-sm sm:text-lg md:text-xl font-mono text-muted-foreground uppercase max-w-3xl mx-auto mb-10 sm:mb-16 tracking-[0.18em] sm:tracking-widest leading-relaxed sm:leading-loose mt-8 sm:mt-12 bg-card border-2 border-primary p-4 sm:p-6 shadow-[6px_6px_0px_0px_hsl(var(--primary))]">
        ...
        {/* CTA Buttons */}
        <div className="flex flex-col sm:flex-row items-stretch justify-center gap-4 sm:gap-6 mb-20 sm:mb-32 w-full max-w-md mx-auto">
          <Link
            href="/auth/sign-in"
            className="flex-1 w-full text-center border-4 border-primary bg-primary text-primary-foreground font-mono font-black uppercase tracking-widest text-base sm:text-lg px-6 sm:px-8 py-4 sm:py-5 hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_hsl(var(--primary))] active:translate-x-0 active:translate-y-0 active:shadow-none transition-all"
          >
            Expose My Texts //
          </Link>
        </div>
        ...
        {/* Analysis Types */}
        <div id="analyses" className="w-full mt-6 sm:mt-12 text-left relative">
          <div className="border-b-4 border-primary pb-4 mb-10 sm:mb-16 flex flex-col sm:flex-row gap-3 justify-between sm:items-end">
            <h2 className="text-3xl sm:text-4xl md:text-6xl font-display font-black uppercase tracking-tight">The Payload_</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-8">
            {[
              ...
              <div key={i} className={`bg-card border-4 border-primary p-5 sm:p-6 hover:-translate-y-2 hover:-translate-x-2 hover:shadow-[10px_10px_0px_0px_hsl(var(--primary))] transition-all flex flex-col items-start group relative overflow-hidden bg-gradient-to-br from-card to-${feature.bg.split(' ')[0]} ${[1, 4, 6].includes(i) ? 'rotate-1 hover:rotate-0' : ''} ${[2, 5].includes(i) ? '-rotate-1 hover:rotate-0' : ''}`}>
                ...
                <h3 className="text-lg sm:text-xl font-black font-display uppercase mb-3 sm:mb-4 text-foreground tracking-wide leading-tight">{feature.title}</h3>
        ...
        {/* Supported Platforms */}
        <div id="platforms" className="w-full mt-24 sm:mt-40 text-left">
           <div className="border-b-4 border-primary pb-4 mb-10 sm:mb-16 flex flex-col sm:flex-row gap-3 justify-between sm:items-end">
            <h2 className="text-3xl sm:text-4xl md:text-6xl font-display font-black uppercase tracking-tight">Data Ingestion_</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
            {[
              ...
              <div key={i} className="bg-background border-4 border-primary p-6 sm:p-8 text-center hover:-translate-y-2 hover:bg-card hover:shadow-[8px_8px_0px_0px_hsl(var(--primary))] transition-all flex flex-col items-center group">
                <platform.icon className={`w-8 h-8 sm:w-10 sm:h-10 ${platform.color} mb-5 sm:mb-6 group-hover:scale-110 transition-transform`} strokeWidth={2.5} />
                <h3 className="text-lg sm:text-xl font-black font-display uppercase tracking-widest mb-3">{platform.title}</h3>
        ...
        {/* Big CTA Component */}
        <div className="w-full mt-24 sm:mt-40 mb-10">
          <div className="bg-primary/5 border-8 border-primary p-6 sm:p-12 md:p-24 shadow-[12px_12px_0px_0px_hsl(var(--primary))] md:shadow-[16px_16px_0px_0px_hsl(var(--primary))] relative overflow-hidden text-left sm:text-center flex flex-col items-center">
            
            <div className="relative z-10 w-full flex flex-col items-center">
              <h2 className="text-3xl sm:text-5xl md:text-[5rem] font-display font-black uppercase mb-6 sm:mb-8 leading-[0.92] sm:leading-[0.9] text-primary">
                STOP GUESSING.<br/>START COMPILING.
              </h2>
              <p className="text-sm sm:text-xl font-mono text-muted-foreground uppercase mb-10 sm:mb-16 max-w-2xl mx-auto tracking-[0.18em] sm:tracking-widest leading-relaxed sm:leading-loose font-bold bg-background border-2 border-primary p-4 text-center">
                Join thousands running diagnostics on their relationships. No cap, it's actually free to try.
              </p>
              <Link
                href="/auth/sign-in"
                className="inline-block border-4 border-primary bg-primary text-primary-foreground font-mono font-black uppercase tracking-widest text-lg sm:text-2xl px-8 sm:px-16 py-5 sm:py-8 hover:-translate-x-2 hover:-translate-y-2 hover:shadow-[12px_12px_0px_0px_hsl(var(--foreground))] active:translate-x-0 active:translate-y-0 active:shadow-none transition-all w-full md:w-auto text-center"
              >
      ...
      {/* Footer */}
      <footer className="border-t-4 border-primary bg-background py-12 sm:py-16 z-10 w-full mt-auto">
        <div className="container mx-auto px-4 md:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 sm:gap-8 border-b-2 border-primary pb-6 sm:pb-8 mb-6 sm:mb-8">
            <div className="flex items-center gap-4">
              ...
              <span className="font-display font-black text-2xl sm:text-3xl uppercase tracking-widest">
                Chatlyzer
              </span>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-0 justify-between items-start sm:items-center w-full font-mono uppercase text-[10px] sm:text-xs font-bold tracking-widest text-muted-foreground group">
